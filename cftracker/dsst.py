...
class DSST(BaseCF):
    def __init__(self,config):
        super(DSST).__init__()
        self.interp_factor = config.interp_factor#0.25
        self.sigma = config.sigma#0.2
        self.lambda_ = config.lambda_#0.01
        self.output_sigma_factor=config.output_sigma_factor#1. / 16
        self.scale_type=config.scale_type#'normal'
        self.scale_config=config.scale_config
        self.padding =config.padding#1
        self.config=config
    ...
    def init(self,first_frame,bbox):
        ...
        if self.scale_type=='normal':
            self.scale_estimator = DSSTScaleEstimator(self.target_sz, config=self.scale_config)#多尺度估计
    ...
    def update(self,current_frame,vis=False):
        ...
        xtf=fft2(xt)
        response=np.real(ifft2(np.sum(self.hf_num*xtf,axis=2)/(self.hf_den+self.lambda_)))#响应计算
        if vis is True:
            self.score=response
            self.win_sz=self.crop_size
        curr=np.unravel_index(np.argmax(response,axis=None),response.shape)#响应最大的位置
    ...
    def get_translation_sample(self,im,center,model_sz,scale_factor,cos_window):
        ...
        out=self.get_feature_map(im_patch)#代码只用hog特征
        out=self._get_windowed(out,cos_window)#加cos窗
        return out
