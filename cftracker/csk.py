...
class CSK(BaseCF):
    ...
    def init(self,first_frame,bbox):
        ...
        self.w,self.h=w,h
        self._window=cos_window((int(round(2*w)),int(round(2*h))))#cos窗/hann窗
        self.crop_size=(int(round(2*w)),int(round(2*h)))
        self.x=cv2.getRectSubPix(first_frame,(int(round(2*w)),int(round(2*h))),self._center)/255-0.5#提取目标框两倍的区域
        self.x=self.x*self._window#加窗，解决频谱泄露问题(频谱泄露和加窗)
        s=np.sqrt(w*h)/16
        self.y=gaussian2d_labels((int(round(2*w)),int(round(2*h))),s)#生成目标框对应的响应
    ...
    def update(self,current_frame,vis=False):
        ...
        z=z*self._window
        self.z=z
        responses=self._detection(self.alphaf,self.x,z)#y=alpha*x
        if vis is True:
            self.score=responses
        curr=np.unravel_index(np.argmax(responses,axis=None),responses.shape)#获取最大位置响应
